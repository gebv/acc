...
func Test100_05CreateHoldRechargeTransfer(t *testing.T) {
	var _, acc1ID, acc2ID, acc3ID int64
	t.Run("Init", func(t *testing.T) {
		_, acc1ID = createAccount(t, "from_i.curr", "ma.user101.main")
		_, acc2ID = createAccount(t, "from_i.curr", "ma.user202.main")
		_, acc3ID = createAccount(t, "from_i.curr", "ma.hold3.main")
		loadAccountBalances(t, "ma")
	})
	...
	var txID int64
	t.Run("HoldRechargeTransfer", func(t *testing.T) {
		c := acca.NewTransferClient(Conn)
		ctx := metadata.NewOutgoingContext(Ctx, metadata.Pairs("foo", "bar"))
	...
	t.Run("CheckBalances", func(t *testing.T) {
		...
		require.EqualValues(t, 20, accounts[acc3ID])
		require.EqualValues(t, 0, res.GetAccounts()[0].Balance)
		require.EqualValues(t, 0, res.GetAccounts()[1].Balance)
		require.EqualValues(t, 20, res.GetAccounts()[2].Balance)
		require.EqualValues(t, 0, res.GetAccounts()[0].BalanceAccepted)
		require.EqualValues(t, 0, res.GetAccounts()[1].BalanceAccepted)
		require.EqualValues(t, 0, res.GetAccounts()[2].BalanceAccepted)
...
func Test100_06CreateHoldWithdrawTransfer(t *testing.T) {
	var _, acc1ID, acc2ID, acc3ID int64
	t.Run("Init", func(t *testing.T) {
		_, acc1ID = createAccount(t, "from_i.curr", "ma.user104.main")
		_, acc2ID = createAccount(t, "from_i.curr", "ma.user204.main")
		_, acc3ID = createAccount(t, "from_i.curr", "ma.hold4.main")
		loadAccountBalances(t, "ma")
	})
	...
	var txID int64
	t.Run("HoldWithdrawTransfer", func(t *testing.T) {
		c := acca.NewTransferClient(Conn)
		ctx := metadata.NewOutgoingContext(Ctx, metadata.Pairs("foo", "bar"))
	...
	t.Run("CheckBalances", func(t *testing.T) {
		...
		require.Len(t, res.Accounts, 3)
		loadAccountBalances(t, "ma")
		require.EqualValues(t, 15, accounts[acc1ID])
		require.EqualValues(t, 20, accounts[acc2ID])
		require.EqualValues(t, 5, accounts[acc3ID])
		require.EqualValues(t, 15, res.GetAccounts()[0].Balance)
		require.EqualValues(t, 20, res.GetAccounts()[1].Balance)
		require.EqualValues(t, 5, res.GetAccounts()[2].Balance)
		require.EqualValues(t, 20, res.GetAccounts()[0].BalanceAccepted)
		require.EqualValues(t, 20, res.GetAccounts()[1].BalanceAccepted)
		require.EqualValues(t, 0, res.GetAccounts()[2].BalanceAccepted)
	})
	...
	t.Run("CheckBalances", func(t *testing.T) {
		...
		loadAccountBalances(t, "ma")
		require.EqualValues(t, 15, accounts[acc1ID])
		require.EqualValues(t, 15, accounts[acc2ID])
		require.EqualValues(t, 0, accounts[acc3ID])
		require.EqualValues(t, 15, res.GetAccounts()[0].Balance)
		require.EqualValues(t, 15, res.GetAccounts()[1].Balance)
		require.EqualValues(t, 0, res.GetAccounts()[2].Balance)
		require.EqualValues(t, 15, res.GetAccounts()[0].BalanceAccepted)
		require.EqualValues(t, 15, res.GetAccounts()[1].BalanceAccepted)
		require.EqualValues(t, 0, res.GetAccounts()[2].BalanceAccepted)
	})
}
